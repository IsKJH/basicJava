 ...
 * 2. 하위클래스 타입의 참조변수를 이용해서 상위클래스에 선언된 멤버변수를 엑세스 할 수 있다.
 * 3. 상위클래스에서 선언된 멤버변수명과 동일한 이름의 멤버변수를 하윜클래스에서 정의하면 하위클래스내부에서는 부모클래스의
 * 	  멤버변수가 아니라 하위클래스의 멤버변수가 인식된다.
 ...
 *   super -> 부모 객체
 * 5.상위클래스의 멤버가 private으로 정의되어 있으면 하위클래스에서 엑세스 할 수 없다.
 *   (상속되지 않는다.)
 */
class Super { // 상위클래스
	int num = 100;
...
//		System.out.println("data=>" + data); // 부모의 private멤버를 엑세스하므로 오류
